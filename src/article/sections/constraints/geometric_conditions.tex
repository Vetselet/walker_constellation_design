...
The problem of ensuring continuous global coverage by a Walker constellation can be 
decomposed into three geometric conditions, illustrated in Figure~\ref{fig:geometric_conditions_overview}. 
These conditions address the co-orbital, retrograde, and polar relative motion of satellites. 
Their joint satisfaction guarantees the absence of coverage gaps in all latitudinal zones.

Geometric conditions for continuous global coverage are initially inherited from the classical street-of-coverage formulations developed for polar constellations \cite{beste1978,rider1985, wertz1999smad}. For near-polar Walker constellations, however, accurate coverage assessment requires the actual orbital inclination to be retained, since the assumption \(i=90^\circ\) may distort the geometry of high-latitude and polar coverage \cite{ulybyshev1999,deweck2004}. Building on inclination dependent extensions of continuous global coverage analysis \cite{ulybyshev1999, huang2021}, the present formulation therefore allows \(i\neq90^\circ\) and evaluates the sufficient accurate geometric conditions for Walker constellations.

Co-orbital planes (Fig.~\ref{fig:coorbital_condition}a) are adjacent planes where spacecraft
move in the same direction. In the Walker constellation, the phase shift between spacecraft in
adjacent planes is fixed, and spacecraft within a plane are separated by equal intervals in
argument of latitude. To ensure continuous overlap between these planes, the maximum allowable
angular distance between them must not exceed \(D\) (\ref{eq:D}):
\begin{equation}\label{eq:D}
    D = \lambda_{\text{street}} + \theta,
\end{equation}

Retrograde planes (Fig.~\ref{fig:retrograde_condition}b) are adjacent planes where spacecraft
move in opposite directions, such as the first and last planes of a Walker constellation.
To ensure continuous overlap between these planes, the streets of coverage must touch, meaning
the maximum allowable angular distance between them must not exceed \(D_{re}\) (\ref{eq:D_re}):
\begin{equation}\label{eq:D_re}
    D_{re} = 2\lambda_{\text{street}}.
\end{equation}

The polar case (Fig.~\ref{fig:polar_condition}c) addresses the uncovered "claw"-shaped region
near the poles, which arises because orbits with inclination \(i < 90^\circ\) do not pass directly
over the poles. To close this region, the streets of coverage must extend all the way to the axis
of rotation.

It is important to note that these conditions are sufficient for the spherical Earth model.
Since the actual Earth's ellipsoid is inscribed within this sphere, satisfying these
inequalities guarantees full coverage for the real ellipsoidal model as well.
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        ...
        \caption{Co-orbital planes: adjacent planes move in the same direction; the street of coverage and satellite's field of view are shown.}
        \label{fig:coorbital_condition}
    \end{subfigure}\hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{pictures/retrograde_condition.jpg}
        \caption{Retrograde planes: adjacent planes move in opposite directions; the streets of coverage must be touched.}
        \label{fig:retrograde_condition}
    \end{subfigure}\hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{pictures/polar_condition.png}
        \caption{Polar case: illustration of the uncovered region around the pole named "claw" and the 
        requirement for the streets of coverage to reach the rotation axis.}
        \label{fig:polar_condition}
    \end{subfigure}
    \caption{Geometric conditions for coverage: (a) co-orbital, (b) retrograde, (c) polar.}
    \label{fig:geometric_conditions_overview}
\end{figure}

\paragraph{Conditions for Co-Orbital Planes}

Equating angular distance between two co-orbital
planes $$i_{\mathrm{rel}}$$ (\ref{eq:i_rel}) to the maximum allowable value $$D$$ (\ref{eq:D}), we obtain the constraint
...
allowable RAAN range. The angular distance between these planes is denoted as $i_{\mathrm{rel}}$.
...
$i_{\mathrm{rel}}$ is:
\begin{equation}
    d = i_{\mathrm{rel}} - \lambda_{\text{street}}.
\end{equation}
...
\begin{equation}\label{eq:phase_shift}
    f_{max} = \frac{\Delta u}{2} + \beta - 2\eta
\end{equation}
\begin{equation}
    f_{min} = \frac{\Delta u}{2} - \beta - 2\eta
\end{equation}
...
\end{equation}
\begin{equation}
    \eta = \arccos \left( \frac{\cos(\frac{\Delta\Omega}{2})}{\cos(\frac{i_{\mathrm{rel}}}{2})} \right)
\end{equation}

By substituting the expressions for \(\eta\) and \(\beta\) into \eqref{eq:phase_shift}, we obtain
...
\begin{equation}
    ...
    - 2 \arccos \left( \frac{\cos\left(\frac{\Delta\Omega}{2}\right)}{\cos\left(
        \frac{i_{\mathrm{rel}}}{2}\right)} \right)
\end{equation}
\begin{equation}
...
\begin{equation}
    i_{\mathrm{rel}}^{(re)} = \pi - i_{\mathrm{rel}}.
\end{equation}
If the condition \(i_{\mathrm{rel}}^{(re)} \leq D_{re}\) holds, the coverage on the retrograde planes remains
...
Equating the angular distance between the first and last planes \(i_{\mathrm{rel}}^{(re)}\) to \(D_{re}\), we
...
For orbits with inclinations less than $90^\circ$, 
...
Under this condition, the streets-of-coverage of all orbital planes pass through the
pole in the worst-case configuration and cover the otherwise uncovered region. This
constraint is imposed on the streets of coverage to account for retrograde orbits.
The traditional condition \(\lambda_{\mathrm{street}} > 0\) \cite{wertz2001ocdm} is
achieved only in the limited case of \(i=\frac{\pi}{2}\).
