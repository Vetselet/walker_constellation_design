...
satisfies two conditions: the angular criterion for being within the spacecraft field of view (FOV), whose effective half-angle is defined by
\eqref{eq:effective_fov}, and the vector criterion for geometric line-of-sight visibility.
...
\begin{equation}\label{eq:point_look_angle}
...
\begin{equation}\label{eq:angular_coverage_criterion}
...
Thus, at a fixed time, a ground point is geometrically covered by a spacecraft if
conditions~\eqref{eq:angular_coverage_criterion} and
\eqref{eq:vector_coverage_criterion} hold simultaneously. When protection of
geostationary systems is required, this pairwise criterion is supplemented by the
mask introduced in Subsection~\ref{subsec:geoprotection}.
